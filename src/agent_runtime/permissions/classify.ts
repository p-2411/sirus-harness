...
// What a tool call is, as far as the gate is concerned. A tool says whether
// it reads or mutates; a shell command has to be parsed, because "run this
// command" says nothing on its own.
...
export function resolveInside(directory: string, target: string): boolean {
...
// The tool's own declared effect decides everything but the shell, whose
// command has to be read. A mutating tool that names a path may only change
// one inside the directory the session owns.
export function classifyToolCall(
  tool: Tool | undefined,
  call: ToolCallBlock,
  directory: string,
): ToolClass {
  // The toolbox refuses a call it cannot name before it reaches the gate, so
  // this is unreachable; the strictest answer is still the right default.
  if (!tool) return 'sensitive';
  if (tool.name === 'RunShell') {
...
// the judge.
export function classifyGit(words: readonly string[]): ToolClass {
...
export function classifySimpleCommand(command: SimpleCommand, directory: string): ToolClass {
