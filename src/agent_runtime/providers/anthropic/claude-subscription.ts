...
interface ClaudeUsageDependencies {
  activeQuery: () => ClaudeUsageQuery | undefined;
...
export function createClaudeSubscriptionUsageReader({
  ...
  return async signal => {
    ...
    let temporary: ClaudeUsageQuery | undefined;
    ...
    try {
      ...
      finishInput?.();
      // Shared queries may be running a turn and must survive a usage timeout.
      temporary?.close();
    }
  };
}

function createClaudeRuntime(profile: string) {
  const sessions = new Map<string, ClaudeSession>();
  const environment = () => subscriptionEnvironment('claude', profile);
  const readUsage = createClaudeSubscriptionUsageReader({
    activeQuery: () => [...sessions.values()].at(-1)?.query,
    createQuery: query,
    environment,
  });
  ...
  return { transport, readUsage };
}
const runtimes = new Map<string, ReturnType<typeof createClaudeRuntime>>();
function runtimeFor(profile = 'default') {
  const key = JSON.stringify([dataDirectory(), profile]);
  let runtime = runtimes.get(key);
  if (!runtime) { runtime = createClaudeRuntime(profile); runtimes.set(key, runtime); }
  return runtime;
}
