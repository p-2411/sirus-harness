...
// changed. It works in the directory its owner was working in. While it runs
// its transcript streams into a temporary file so the owner can look in on it. Only the owner can see or steer a run; this module
// also keeps a process-wide index so the UI can show every run.
...
// Every run in the process, for the UI; ownership lives on the SessionAgent.
...
export function startSubagent(
  ...
  installExitCleanup();
  completions.set(run.id, execute(run, options.signal));
  notifyListeners();
  return run;
}
